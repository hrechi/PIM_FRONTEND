  ...
  static Future<dynamic> _handleResponse(
    ...
    if (response.statusCode == 401 && withAuth) {
      ...
      if (refreshed) {
        ...
        final dynamic retryData = jsonDecode(retryResponse.body);
        if (retryResponse.statusCode >= 200 && retryResponse.statusCode < 300) {
          return retryData;
        }
        
        String errorMessage = 'Request failed';
        if (retryData is Map && retryData.containsKey('message')) {
          errorMessage = retryData['message'].toString();
        }
        
        throw ApiException(errorMessage, retryResponse.statusCode);
      }
    ...
    if (data is Map && data.containsKey('message')) {
      ...
      errorMessage = message is List ? message.first.toString() : message.toString();
    }
    
    throw ApiException(errorMessage, response.statusCode);
  }

  static dynamic _handleUploadResponse(http.Response response) {
    final dynamic data = jsonDecode(response.body);
