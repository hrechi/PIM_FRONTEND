...
class ApiService {
  ...
  static String get baseUrl {
    ...
    if (kIsWeb) {
      return 'http://192.168.142.3:$port/api';
    }
  ...
  static String get mediaBaseUrl {
    final host = AppConfig.serverHost;
    final port = AppConfig.serverPort;
    if (kIsWeb) return 'http://192.168.142.3:$port';
    if (Platform.isAndroid) {
      final isEmulator = host == 'localhost' || host == '127.0.0.1';
      return isEmulator ? 'http://192.168.142.3:$port' : 'http://$host:$port';
    }
    if (host == 'localhost' || host == '127.0.0.1')
      return 'http://192.168.142.3:$port';
