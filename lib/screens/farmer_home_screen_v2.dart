...
class _FarmerHomeScreenV2State extends State<FarmerHomeScreenV2> {
  ...
  final TextEditingController _materialNotesController = TextEditingController();
        ...
        if (_assignedMaterials.isEmpty)
          ...
          Row(
            children: [
              Expanded(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                      color: AppColorPalette.charcoalGreen.withValues(alpha: 0.2),
                    ),
                  ),
                  child: DropdownButton<AssetItem>(
                    isExpanded: true,
                    underline: const SizedBox(),
                    hint: Text('Select material...', style: AppTextStyles.bodySmall()),
              ...
              const SizedBox(width: 12),
              ElevatedButton.icon(
                onPressed: () => _showStartSessionDialog(_assignedMaterials.first),
  ...
  Widget _buildMaterialCard(AssetItem material) {
    final isActive = _activeMaterialSession?['assetId']?.toString() == material.id;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _getStatusColor(material.status).withValues(alpha: 0.2),
                    ...
                    child: Text.rich(
                      TextSpan(
                        children: [
                          ...
                          TextSpan(
                            text: material.operatingHours?.toStringAsFixed(1) ?? 'N/A',
                            style: AppTextStyles.bodySmall(),
                          ),
                        ],
                      ),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('${material.brand} • ${material.model ?? 'N/A'}',
                style: AppTextStyles.bodySmall()),
            const SizedBox(height: 16),
            Text('Start time: ${DateFormat('MMM dd, yyyy - hh:mm a').format(DateTime.now())}',
                style: AppTextStyles.caption()),
          ],
        ),
        actions: [
  ...
  void _showCheckInDialog(AssetItem material) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('Return Material: ${material.name}', style: AppTextStyles.h4()),
        content: Form(
